...
public class MeleeWeaponTrail : MonoBehaviour
{
 ...
 //       Setup();
    ...
    void Setup()
    {
        ...
        {
            _lastPosition = _mainObject.localPosition;
            _trailObject = new GameObject("Trail");
            _trailObject.transform.parent = _mainObject.parent;
            //_trailObject.transform.parent = null;
            _trailObject.transform.localPosition = Vector3.zero;
            _trailObject.transform.localRotation = Quaternion.identity;
    ...
    void FixedUpdate()
    {
        ...
        {
            DoEmission(Time.fixedDeltaTime);
        }
    }

    void DoEmission(float deltaTime) {
		if (!_use)
		{
			return;
		}
		...
		float theDistanceSqr = (_lastPosition - _mainObject.localPosition).sqrMagnitude;
		if (_emit)
		{
			if (theDistanceSqr > _minVertexDistanceSqr)
			{
				...
				{
					Point p = new Point();
					p.basePosition = GetBasePosition();
					p.tipPosition = GetTipPosition();
				...
				{
					_points[_points.Count - 1].basePosition = GetBasePosition();
					_points[_points.Count - 1].tipPosition = GetTipPosition();
					//_points[_points.Count - 1].timeCreated = Time.time;

#if USE_INTERPOLATION
					_smoothedPoints[_smoothedPoints.Count - 1].basePosition = GetBasePosition();
					_smoothedPoints[_smoothedPoints.Count - 1].tipPosition = GetTipPosition();
#endif
				}
			}
			else
		...
		if (pointsToUse.Count > 1)
		{
			...
			for (int n = 0; n < pointsToUse.Count; ++n)
			{
                ...
                {
                    float gadientTime = Mathf.Clamp01(time);
                    color = _colorGradient.Evaluate(gadientTime);
                }
    ...
    {
        return _base.localPosition + _baseOffset.x * Vector3.right + _baseOffset.y * Vector3.up + _baseOffset.z * Vector3.forward;
    }

    Vector3 GetTipPosition()
    {
        return _tip.localPosition + _tipOffset.x * Vector3.right + _tipOffset.y * Vector3.up + _tipOffset.z * Vector3.forward;
    }
